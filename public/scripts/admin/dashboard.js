async function dashboard() {
      ...
      const sumByProduct = arr => arr.reduce((acc, r) => { acc[r.product_id] = (acc[r.product_id]||0) + r.amount; return acc; }, {});
      const out7Sum = sumByProduct(outs7), in7Sum = sumByProduct(ins7);
      const allIds7 = Array.from(new Set([...Object.keys(out7Sum), ...Object.keys(in7Sum)]));
      ...
      const sortedOut7 = Object.entries(out7Sum).sort((a,b) => b[1] - a[1]).slice(0,5);
      new Chart(document.getElementById('c2'), {
        type: 'bar',
        data: { labels: sortedOut7.map(e => prodMap[e[0]].name), datasets: [{ label: 'OUT', data: sortedOut7.map(e => e[1]) }] },
      ...
      new Chart(document.getElementById('c4'), {
        type: 'pie',
        data: { labels: ['IN','OUT'], datasets: [{ data: [ ins.reduce((s,r)=>s+r.amount,0), outs.reduce((s,r)=>s+r.amount,0) ] }] },
        options: { responsive: true }
      });

      // 5) Daily IN/OUT Trend
      const days = Array.from({length:7}).map((_,i)=>{ const d=new Date(); d.setDate(d.getDate()-6+i); return d.toISOString().slice(0,10); });
      const outsByDay = days.map(d => outs.filter(r => r.date.slice(0,10)===d).reduce((s,r)=>s+r.amount,0));
      const insByDay  = days.map(d => ins.filter(r => r.date.slice(0,10)===d).reduce((s,r)=>s+r.amount,0));
      new Chart(document.getElementById('c5'), {
        type: 'line',
        data: { labels: days, datasets: [
          { label: 'Daily IN',  data: insByDay },
          { label: 'Daily OUT', data: outsByDay }
        ] },
      ...
      new Chart(document.getElementById('c6'), {
        type: 'bar',
        data: { labels: turnoverIds.map(id=>prodMap[id].name), datasets: [{ label:'Turnover', data: turnoverIds.map(id=> out7Sum[id]/avgStock(prodMap[id]) ) }] },
        options: { responsive:true, scales:{ y:{ beginAtZero:true } } }
      });
    }
