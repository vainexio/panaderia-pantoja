...
async function dashboard() {
  ...
  (function () {
    ...
    makeStatCard('Total Products', `<p style="font-size:3rem; font-weight:bold; color:white; margin:0;">${totalProducts}</p>`, true, true);
    makeStatCard('Total Stock', `<p style="font-size:3rem; font-weight:bold; color:white; margin:0;">${totalStock}</p>`, true, true);

    const lowStockProducts = products.filter(p => p.quantity <= 10);
    const highStockProducts = products.filter(p => p.quantity >= 50);

    makeStatCard('Understock Products', lowStockProducts.length ? `<ul>${lowStockProducts.map(p => `<li>${p.name} (${p.quantity})</li>`).join('')}</ul>` : `<p>None</p>`);
    makeStatCard('Overstock Products', highStockProducts.length ? `<ul>${highStockProducts.map(p => `<li>${p.name} (${p.quantity})</li>`).join('')}</ul>` : `<p>None</p>`);
  })();
        ...
        .forEach(p => {
          ...
          new Chart(ctx, {
            ...
            data: {
              labels: labels,
              datasets: [
                { label: 'IN', data: labels.map(d => ins[d]), fill: false, borderColor: 'green' },
                { label: 'OUT', data: labels.map(d => outs[d]), fill: false, borderColor: 'red' }
              ]
            },
  ...
  downloadForm.addEventListener("submit", (e) => {
    ...
    isCooldown = true;
    setTimeout(() => { isCooldown = false; }, 30000);
  });
}
